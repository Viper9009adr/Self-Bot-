...
export type MediaRouteTarget = 'local' | 'openai' | 'nvidia-nim' | 'comfyui' | 'google' | 'unavailable';
...
export const MEDIA_CAPABILITY_UNAVAILABLE_MESSAGES: Record<MediaCapability, string> = {
  image: 'Image capability not configured. Set LOCAL_COMFYUI_URL, NVIDIA_NIM_API_KEY, GOOGLE_API_KEY, LOCAL_IMAGE_URL, or OPENAI_API_KEY.',
...
export function resolveMediaCapabilityRoutes(config: Config): MediaCapabilityRoutes {
  ...
  const hasGoogle = !!config.llm.googleApiKey;
  const hasComfyUI = !!(config.llm.localComfyuiUrl && config.media?.comfyuiWorkflowPath);

  return {
    image: hasComfyUI ? 'comfyui' : (hasNvidiaNimImage ? 'nvidia-nim' : (hasGoogle ? 'google' : (hasLocalImage ? 'local' : (hasOpenAI ? 'openai' : 'unavailable')))),
